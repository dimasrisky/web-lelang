...
<div class="w-screen h-screen flex justify-center items-center">
    <form action="{{ route('barang.update', ['id' => $dataBarang->id ]) }}" method="post" enctype="multipart/form-data" class="w-[70%] mx-auto">
        @csrf
        @method('PUT')
        <h1 class="font-poetsen text-[48px] text-center">Edit Wisata</h1>
        <div class="flex flex-col gap-2 mt-4">
            <label for="nama_barang" class="font-inter text-[17px] font-semibold">Nama Barang</label>
            <input type="text" id="nama_barang" name="nama_barang" placeholder="Masukkan Nama Barang..." class="border border-[#E4E4E4] text-xs px-3 py-2" value="{{ $dataBarang->nama_barang }}">
        </div>
        <div class="flex flex-col gap-2 mt-4">
            <label for="deskripsi_barang" class="font-inter text-[17px] font-semibold">Deskripsi</label>
            <input type="text" id="deskripsi_barang" name="deskripsi_barang" placeholder="Deskripsi..." value="{{ $dataBarang->deskripsi_barang }}" class="border border-[#E4E4E4] text-xs px-3 py-2">
        </div>
        <div class="flex flex-col gap-2 mt-4">
        ...
        <div class="flex flex-col gap-2 mt-4">
            ...
            <input type="number" id="harga" name="harga" placeholder="Masukkan harga Barang..." value="{{ $dataBarang->harga_awal }}" class="border border-[#E4E4E4] text-xs px-3 py-2">
        </div>
        <div class="flex flex-col gap-2 mt-4">
        ...
        <div class="flex flex-col gap-2 mt-4">
            <input type="hidden" name="oldGambar" value="{{ $dataBarang->gambar }}">
        ...
        <img src="{{ asset('/storage/produk/' . $dataBarang->gambar) }}" alt="gambar" class="w-[15rem]">
        <button type="submit" class="px-6 py-2 mt-4 text-white text-xs font-inter font-semibold bg-blue-600 rounded-md">Ubah Barang</button>
    </form>
</div>
